...
// Helper : savoir si l'utilisateur est admin (global OU dans l'équipe)
const isTeamAdmin = (team, user) => {
  if (!user) return false;

  // Global admin = toujours autorisé
  ...
  // Si role manquant (anciens enregistrements), on les considère admin pour ne rien casser
...
exports.createTeam = async (req, res) => {
  try {
    ...
    // Seuls les admins globaux peuvent créer des équipes
    ...
    // Création avec le créateur comme ADMIN de l'équipe
    ...
    // Ajout d'autres membres (role = member)
    ...
    // Ajout du créateur dans ses équipes
...
exports.getTeam = async (req, res) => {
  try {
    const team = await Team.findById(req.params.id)
      .populate(
        'members.user',
        'firstName lastName email avatar bio phone'
      )
      .populate('createdBy', 'firstName lastName');
...
// @access  Private (admin équipe ou global)
